...
classification.
%}
PLI_LH = load(input_filename).pli_lh_beta;
PLI_RH = load(input_filename).pli_rh_beta;
PLI_BF = load(input_filename).pli_bf_beta; 
PLI_T = load(input_filename).pli_t_beta;


% The whole dataset contains the data of all the events
X_ALPHA = [PLI_RH; PLI_LH; PLI_BF; PLI_T];
...
% "Both Feet" epochs
y_ALPHA = [ones([size(PLI_RH,1),1])*-1;
    zeros([size(PLI_LH,1),1]);
    ones([size(PLI_BF,1),1]); 
    ones([size(PLI_T,1),1])*2];
...
Save the output.
%}
save(strcat(output_path,subject, '/BETA/KFOLD5/', '/pli_beta_', subject, '_',output_extension), 'resultsBayes');